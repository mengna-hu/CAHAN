...
from utils import load_BioNet_data, EarlyStopping, index_generator, parse_minibatch
...
def main(args):
    adjlists_ua, adjM, type_mask, train_val_test_pos_drug_dis, train_val_test_neg_drug_dis = load_BioNet_data()
...
#     print("The main parameters are as follows:")
#     print(f"batch_size:{args['batch_size']} | weight_decay:{args['weight_decay']} | learng_rate: {args['lr']} | feature_dim: {args['out_size']}")
#     print(f"attention_head:{args['num_heads']} | patience: {args['patience']} | repeat:{args['repeat']} | dropout:{args['dropout']}")
    ...
    # for i in range(1):
        ...
        # np.savez(f'./record/record_rd/CAHAN_prediction_result_{j}.npz', y_true=y_true_test, y_pred=y_proba_test)
        # np.savez(f'./record/record_rd/CAHAN_embedding2vis_InfoMaxFusv1_{j}.npz', drug=np.concatenate(drug_embedding_list, axis=0),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-postfix',default='rd')  
